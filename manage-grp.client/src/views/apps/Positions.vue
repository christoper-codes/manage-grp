...
import { positionsSearchSchema } from '@/validation/positions/search';
import { positionStoreOrUpdateSchema } from '@/validation/positions/storeOrUpdate';
...
// Form for searching positions
const { handleSubmit: handleSearchSubmit } = useForm({ validationSchema: positionsSearchSchema(t) });
...
// Form for storing positions
const { handleSubmit: handleStoreOrUpdateSubmit, resetForm } = useForm({
  validationSchema: positionStoreOrUpdateSchema(t),
  initialValues: {
    isActive: true,
  },
});
...
const abbreviation = useField('abbreviation');
...
const headers = ref([
    ...
    { title: t('ABBREVIATION_HEADER'), key: 'abbreviation' },
...
const isPositionStore = ref(true);
const states: Ref<any[]> = ref([]);
const municipalities: Ref<any[]> = ref([]);
const positions: Ref<any[]> = ref([]);
const loading: Ref<boolean> = ref(false);
const search = ref();
const position: Ref<object> = ref({});
const dialog = ref(false);
const dialogDelete = ref(false);

// Arrow functions
const formTitle = computed(() => {
    return isPositionStore.value ? t('ADD_NEW_ITEM_FIELD') : t('UPDATE_ITEM_FIELD');
});

const storePosition = () => {
  resetForm();
  isPositionStore.value = true;
}

const editPosition = (item: any) => {
    isPositionStore.value = false;
    id.value.value = item.id;
    dependencyId.value.value = item.dependencyId;
    name.value.value = item.name;
    abbreviation.value.value = item.abbreviation;
...
const handleSearchPositions = handleSearchSubmit(async (values: Record<string, any>) => {
  await indexPositions(values.searchDependencySelected.id);
});

const indexPositions = async (id:number) => {
  positions.value = await positionsStore.positionsByDependency(id, loading, t);
};

const handleStoreOrUpdatePosition = handleStoreOrUpdateSubmit(async (values: Record<string, any>) => {
  await positionsStore.storeOrUpdatePosition(values, loading, t, isPositionStore.value);
  await indexPositions(values.dependencyId);
  close();
});

const handleDeletePosition = async () => {
  await positionsStore.deletePosition(position.value.id, loading, t);
  await indexPositions(position.value.dependencyId);
  position.value = {};
  close();
}
...
<template>
    <div data-aos="fade-left" data-aos-duration="1500">
      <BaseBreadcrumb title="POSITIONS"></BaseBreadcrumb>
      <div class="tw-pt-7 tw-pb-1 tw-px-7 tw-rounded-xl tw-bg-container-bg tw-shadow-sm tw-w-full mb-8 tw-flex tw-items-center tw-gap-5">
        ...
        <v-select
          ...
          ></v-select>
          <v-btn @click="handleSearchPositions" :loading="loading" :disabled="loading" class="!tw-bg-gradient-to-r !tw-from-primary !tw-to-secondary !tw-text-white !tw-mb-6" variant="flat" size="large" dark>{{ $t('SEARCH_FIELD') + ' ' + $t('POSITIONS') }}</v-btn>
      </div>
                    ...
                    <template v-slot:item.ITEM_IMAGE_HEADER="{ item }">
                        <div class="d-flex gap-3 align-center">
                            ...
                            <div>
                                ...
                                <p class="textSecondary">{{ item.abbreviation }}</p>
                            </div>
                        </div>
                    </template>
                            ...
                            <v-dialog v-model="dialog" max-width="800px">
                                ...
                                <v-card>
                                    <v-card-title class="pa-4 !tw-bg-gradient-to-r !tw-from-primary !tw-to-secondary !tw-text-white">
                                      ...
                                      <div class="tw-grid tw-grid-cols-2 tw-w-full tw-gap-5 tw-px-10 tw-pt-10">
                                        <v-select
                                          ...
                                          persistent-hint
                                        ></v-select>
                                        <v-text-field clearable v-model="abbreviation.value.value" :label="$t('ABBREVIATION_FIELD')" :error-messages="abbreviation.errorMessage.value"></v-text-field>
                                    ...
                                    <v-card-actions class="pa-4">
                                        ...
                                        <v-btn color="success" :loading="loading" :disabled="loading" variant="flat" dark @click="handleStoreOrUpdatePosition"> {{ $t('SAVE_FIELD') }} </v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-dialog>
                            <v-dialog v-model="dialogDelete" max-width="600px">
                                <v-card>
                                    ...
                                    <v-card-actions class="!tw-my-3">
                                        <v-spacer></v-spacer>
                                        <v-btn color="error" variant="flat" dark @click="close">{{ $t('CANCEL_FIELD') }}</v-btn>
                                        <v-btn color="success" variant="flat" dark @click="handleDeletePosition">{{ $t('CONFIRM_FIELD') }}</v-btn>
                                        <v-spacer></v-spacer>
                                    </v-card-actions>
                                </v-card>
                            </v-dialog>
