...
const filteredSearchSugg = computed(() => {
    ...
    <v-menu :close-on-content-click="false">
        ...
        <v-sheet width="360" elevation="10" rounded="md">
            <div class="d-flex align-center justify-space-between pa-5">
                <v-text-field v-model="searchValue" placeholder="Search" color="primary" append-inner-icon="mdi-magnify" density="compact" variant="outlined" hide-details></v-text-field>
            </div>
            <v-divider></v-divider>
            <h5 class="text-h5 mt-4 px-5 pb-4">{{ $t('QHICK_PAGE_LINKS') }}</h5>
            <perfect-scrollbar style="height: 380px">
                <v-list class="pt-0 pb-5" lines="two">
                    <v-list-item
                        :value="item"
                        v-for="(item, index) in filteredSearchSugg"
                        :key="index"
                        :to="item.href"
                        color="primary"
                        class="px-5 py-2"
